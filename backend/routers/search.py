...
from elastic.search import get_document_by_id, get_filter_options, search_documents, suggest_completions
from models.schemas import DocumentDetail, HealthResponse, IndexResponse, SearchFacets, SearchResponse, SuggestResponse
...
@router.post("/index", response_model=IndexResponse, tags=["Admin"])
async def reindex(
    force: bool = Query(False, description="Delete and recreate index before indexing"),
    es: Elasticsearch = Depends(get_es),
):
    result = index_documents(es, data_path="data/textbook.json", force_recreate=force)
    return IndexResponse(
        indexed=result["indexed"],
        failed=result["failed"],
        message=f"Successfully indexed {result['indexed']} documents."
        if result["failed"] == 0
...
async def health_check(es: Elasticsearch = Depends(get_es)):
    connected = es.ping()
